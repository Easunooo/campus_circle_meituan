...
export const ClubDetail: React.FC<Props> = ({ club, onClose, onLike, onSubmit, isLiked }) => {
  ...
  return (
      ...
      >
        <button 
          onClick={onClose}
          className="fixed top-8 left-6 z-[210] w-10 h-10 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center text-on-surface border border-black/5 shadow-xl active:scale-90 transition-all"
        >
          <X size={20} strokeWidth={2.5} />
        </button>

        <div className="flex-1 overflow-y-auto no-scrollbar pt-6 pb-12 px-4">
          <motion.div
            initial={{ y: 50, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            className="w-full bg-white rounded-[2.5rem] shadow-2xl relative z-20 px-8 pt-12 pb-8 flex flex-col border border-black/5"
          >
            <div className="mb-3">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-6 mb-12">
              {[
                ...
                { label: '需要作品', value: club.requiresAttachment ? '需传作品' : '无需作品' },
              ].map((item, i) => (
