    ...
    <div className="flex-1 relative overflow-hidden bg-surface">
      ...
      <div className="absolute inset-0 z-10 overflow-y-auto no-scrollbar pt-20 pb-40">
        <section className="flex flex-col items-center mb-10 px-8">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1 mt-6">
              <div className="text-[13px] font-bold text-on-surface/80 tracking-tight">{user.college} · {user.major}</div>
              <div className="flex items-center gap-3 text-[11px] font-medium text-on-surface-variant/40 tracking-wider">
                <span>{user.enrollmentYear}级</span>
                <span className="w-1 h-1 rounded-full bg-black/5" />
                <span>{user.studentId}</span>
              </div>
              <div className="mt-4 flex flex-col items-center gap-1">
                <div className="text-[12px] font-medium text-on-surface-variant/60">{user.email}</div>
                <div className="text-[12px] font-bold text-on-surface-variant/30 tracking-[0.2em]">{user.phone}</div>
              </div>
            </div>
          </motion.div>
        </section>
        ...
        <div className="px-6 space-y-8">
          ...
          >
            <h3 className="px-5 text-[11px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40 mb-4">账号管理</h3>
            <div className="bg-white/40 backdrop-blur-3xl rounded-[2.5rem] p-2 border border-white/60 shadow-sm overflow-hidden">
          ...
          >
            <h3 className="px-5 text-[11px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40 mb-4">更多信息</h3>
            <div className="bg-white/40 backdrop-blur-3xl rounded-[2.5rem] p-2 border border-white/60 shadow-sm overflow-hidden">
          ...
          >
            <button className="w-full py-5 rounded-[2rem] bg-on-surface/5 hover:bg-on-surface/10 text-on-surface-variant/60 font-headline font-black text-[15px] transition-all duration-300 active:scale-[0.98]">
              安全退出
            </button>
            <p className="mt-10 text-[10px] font-bold uppercase tracking-[0.3em] text-neutral-400 opacity-60">CampusCircle · Release 2.4.0</p>
          </motion.div>
        </div>
      </div>
    </div>
...
const MenuLink = ({ icon, label, color, first, onClick }: any) => (
  <button 
    ...
    first ? "rounded-t-[2.2rem]" : "rounded-b-[2.2rem]"
  )}>
    <div className="flex items-center gap-4">
      <div className={cn("flex items-center justify-center transition-transform", color)}>
        {React.cloneElement(icon as React.ReactElement, { strokeWidth: 2, size: 20 })}
      </div>
      <span className="font-bold text-[15px] text-on-surface/80 group-hover:text-on-surface transition-colors">{label}</span>
    </div>
    <div className="flex items-center justify-center opacity-20 group-hover:opacity-40 transition-opacity">
      <ChevronRight size={18} strokeWidth={3} className="text-on-surface" />
    </div>
  </button>
);
